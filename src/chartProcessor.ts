...
import type { H2HMatch, ProcessedMatch, Manager, ManagerChartModel, ChartData } from './types.js';

export class ChartProcessor {
    private matches: ProcessedMatch[];
    private managers: Map<string, Manager>;  // Changed from Set to Map
    ...
    constructor(h2hMatches: H2HMatch[]) {
     ...
     */
    private convertMatches(h2hMatches: H2HMatch[]): ProcessedMatch[] {
    ...
    public process(): ChartData {
        ...
        const matchPointsByGW = new Map<string, Map<number, number>>();
        
        // Track cumulative absolute FPL points by actual gameweek number
        const absolutePointsByGW = new Map<string, Map<number, number>>();
        ...
        for (const match of happenedMatches) {
            ...
            for (let prevGW = gw - 1; prevGW >= minGameWeek; prevGW--) {
                ...
                if (man1PrevMatch !== 0 && man2PrevMatch !== 0) break;
            }
        ...
        const matchPointsWeekly = new Map<string, number[]>();
        const absolutePointsWeekly = new Map<string, number[]>();
        const relativeWeekly = new Map<string, number[]>();
        ...
        const absoluteManagers: ManagerChartModel[] = [];
        const relativeManagers: ManagerChartModel[] = [];
